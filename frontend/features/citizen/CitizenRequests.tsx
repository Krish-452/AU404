...
import { ShieldAlert, Clock, ChevronRight, CheckCircle, XCircle, FileText, Info } from 'lucide-react';
import { AccessRequest, RequestStatus } from '../../types';
import VerifyOtpModal from '../../components/VerifyOtpModal';
import { useAuth } from '../../context/AuthContext';

const CitizenRequests: React.FC = () => {
  const { sendMfa } = useAuth();
  ...
  const handleAction = async (id: string, action: 'authorize' | 'block', otp?: string) => {
    // High Risk: Block requires OTP
    if (action === 'block' && !otp) {
      setSelectedRequest(id);
      // Trigger OTP send
      await sendMfa('citizen@idtrust.gov');
      setModalOpen(true);
      return;
    }
    ...
    try {
      // Optimistic Update
      setRequests(prev => prev.map(r => r.id === id ? { ...r, status: action === 'authorize' ? RequestStatus.APPROVED : RequestStatus.DENIED } : r));

      const endpoint = action === 'authorize' ? '/citizen/authorize' : '/citizen/block';
      await import('../../services/api').then(m => m.default.post(endpoint, { requestId: id, otp }));

    } catch (error) {
      console.error("Action failed", error);
      // Rollback
      setRequests(prev => prev.map(r => r.id === id ? { ...r, status: RequestStatus.PENDING } : r));
    }
  };

  const onOtpVerified = (otp: string) => {
    if (selectedRequest) {
      handleAction(selectedRequest, 'block', otp);
      setModalOpen(false);
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-700 pb-20 transition-colors">
            ...
            <div className="p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4">
                {req.status === RequestStatus.PENDING ? (
                  <>
                    <button onClick={() => handleAction(req.id, 'authorize')} className="w-full sm:w-auto px-8 py-4 bg-slate-900 dark:bg-indigo-600 text-white font-black text-[10px] uppercase tracking-[0.2em] rounded-xl hover:bg-black dark:hover:bg-indigo-700 transition-all shadow-lg dark:shadow-none">Authorize Share</button>
                    <button onClick={() => handleAction(req.id, 'block')} className="w-full sm:w-auto px-8 py-4 bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-500 font-black text-[10px] uppercase tracking-[0.2em] rounded-xl hover:bg-rose-50 dark:hover:bg-rose-950/30 hover:text-rose-500 transition-all border border-slate-100 dark:border-slate-700">Block Request</button>
                  </>
                ) : (
      ...
      <VerifyOtpModal
        ...
        actionName="Block Access Request"
      />
    </div>
  );
};
