          ...
          <div key={req.id} className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden hover:shadow-xl hover:shadow-slate-100/50 dark:hover:shadow-none transition-all group">
            <div className="p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="flex items-start gap-6">
                <div className={`h-16 w-16 min-w-[64px] rounded-2xl flex items-center justify-center transition-colors ${
                  req.status === RequestStatus.PENDING ? 'bg-orange-50 dark:bg-orange-950/30 text-orange-500' :
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4">
                {req.status === RequestStatus.PENDING ? (
                  <>
                    <button className="w-full sm:w-auto px-8 py-4 bg-slate-900 dark:bg-indigo-600 text-white font-black text-[10px] uppercase tracking-[0.2em] rounded-xl hover:bg-black dark:hover:bg-indigo-700 transition-all shadow-lg dark:shadow-none">Authorize Share</button>
                    <button className="w-full sm:w-auto px-8 py-4 bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-500 font-black text-[10px] uppercase tracking-[0.2em] rounded-xl hover:bg-rose-50 dark:hover:bg-rose-950/30 hover:text-rose-500 transition-all border border-slate-100 dark:border-slate-700">Block Request</button>
                  </>
                ) : (
                  <div className={`px-6 py-3 rounded-xl flex items-center gap-3 text-[10px] font-black uppercase tracking-widest transition-colors ${
                    req.status === RequestStatus.APPROVED ? 'bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400' : 'bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400'
                  }`}>
