...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen, role }) => {
  const getNavItems = () => {
    switch (role) {
      case UserRole.CITIZEN:
        return [
          { icon: LayoutDashboard, label: 'Portal Dashboard', path: '/dashboard', hasSub: true },
  ...
  return (
    <>
      <div
        className={`fixed inset-0 bg-black/80 backdrop-blur-md z-[90] transition-opacity duration-300 md:hidden ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'
          }`}
        onClick={() => setIsOpen(false)}
      />

      <aside className={`
        fixed inset-y-0 left-0 z-[100] transform text-white transition-all duration-300 ease-in-out bg-black
        ${isOpen ? 'w-72 translate-x-0' : 'w-0 -translate-x-full md:translate-x-0 md:w-0'}
        md:relative md:translate-x-0 h-full flex flex-col overflow-hidden border-r border-white/5
      `}>
        <div className="p-8 border-b border-white/5 flex items-center justify-between shrink-0 bg-black">
          ...
          <button onClick={() => setIsOpen(false)} className="md:hidden p-2 hover:bg-white/10 rounded-lg transition-colors">
            <X size={18} />
          </button>
        </div>
          ...
          {getNavItems().map((item) => (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                {item.hasSub && <ChevronRight size={14} className="rotate-90 text-indigo-400" />}
              </div>
            </NavLink>
          ))}
        </nav>

        <div className="p-8 text-center text-[8px] font-black text-slate-600 uppercase tracking-widest border-t border-white/5 shrink-0 bg-black">
